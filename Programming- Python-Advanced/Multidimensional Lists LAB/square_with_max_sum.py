...
def get_coordinate_best_submatrix_sum(matrix, submatrix_size):
    best_row_index = 0
    best_column_index = 0
    ...
    for row_index in range(len(matrix)- submatrix_size + 1):
        for col_index in range(len(matrix[row_index])- submatrix_size + 1):
            curr_sum = get_sum_of_submatrix(matrix, row_index, col_index, submatrix_size)
...
matrix = read_matrix()
SUBMATRIX_SIZE = 2
coordinates = get_coordinate_best_submatrix_sum(matrix, SUBMATRIX_SIZE)
