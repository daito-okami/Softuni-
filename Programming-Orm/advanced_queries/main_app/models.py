...
class Invoice(models.Model):
    ...
    def get_invoices_with_prefix(cls, prefix):
        return cls.objects.select_related('billing-info').filter(invoice_number__startswith=prefix)
...
class Task(models.Model):
    ...
    def overdue_high_priority_tasks(cls):
        return cls.objects.filter(
            Q(priority="High") & Q(is_completed=False) & Q(completion_date__gte=F('creation_date'))
        )
