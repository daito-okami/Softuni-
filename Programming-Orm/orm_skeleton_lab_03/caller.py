...
def filter_authors_by_nationalities(nationality):
    ...
    result = [author.biography if author.biography
...
def filter_authors_by_birth_year(first_year, second_year):
    authors = Author.objects.filter(birth_date__range=(first_year, second_year)).order_by('-birth_date')
...
def change_reviewer_name(reviewer_name, reviewer_new_name):
    review = Review.objects.filter(reviewer_name= reviewer_name).update(reviewer_name=reviewer_new_name)
